...
struct TrainingState
{
    ...
    Adam<67U, 4U, float> optimizer{1.0e-2F};
    ...
    float loss{};
    float patterns[iris_data::sample_count][input_size]{};
...
int main()
{
    ...
    {
        iris_data::normalized_features(sample, state->patterns[sample]);
    }

    Tape<float> &tape = state->arena.tape();
    tape.register_operator<FullyConnectedOperator<float>>();
    tape.register_operator<ReluOperator<float>>();
    tape.register_operator<SoftmaxOperator<float>>();
    tape.register_operator<CrossEntropyOperator<float>>();

    BufferView<float> input = tape.input(state->input);
    MatrixView<float> hidden_weight = tape.parameter(state->model.hidden_weight);
    BufferView<float> hidden_bias = tape.parameter(state->model.hidden_bias);
    MatrixView<float> output_weight = tape.parameter(state->model.output_weight);
    BufferView<float> output_bias = tape.parameter(state->model.output_bias);
    BufferView<float> hidden_linear = tape.output(state->hidden_linear);
    BufferView<float> hidden = tape.output(state->hidden);
    BufferView<float> logits = tape.output(state->logits);
    BufferView<float> probability = tape.output(state->probability);
    BufferView<float> target = tape.input(state->target);
    BufferView<float> loss = tape.output(state->loss);
    ...
    for (std::size_t epoch = 0; epoch < epoch_count; ++epoch)
    {
        ...
        for (std::size_t position = 0; position < training_count; ++position)
        {
            ...
            for (std::size_t feature = 0; feature < input_size; ++feature)
                state->input[feature] = state->patterns[sample][feature];
    ...
    {
        RecordingScope<float> inference(tape, false);
    ...
    delete state;
    // The embedded runtime loops if main returns; use the semihosting exit.
    std::exit(0);
}
