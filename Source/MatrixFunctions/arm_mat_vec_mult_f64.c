...
ARM_DSP_ATTRIBUTE void arm_mat_vec_mult_f64(
    const arm_matrix_instance_f64   *pSrcMat,
    const float64_t                 *pSrcVec,
    float64_t                       *pDstVec)
{
    uint32_t         numRows = pSrcMat->numRows;
    uint32_t         numCols = pSrcMat->numCols;
    const float64_t *pSrcA = pSrcMat->pData;
    const float64_t *pInA0;
    const float64_t *pInA1;
    float64_t       *px;
    int32_t          row;
    ...
    while (row >= 4)
    {
        const float64_t     *pInA2, *pInA3;
        float64_t const    *pSrcA0Vec, *pSrcA1Vec, *pSrcA2Vec, *pSrcA3Vec, *pInVec;
        f64x4_t            vecIn, acc0, acc1, acc2, acc3;
        float64_t const     *pSrcVecPtr = pSrcVec;
         ...
         */
        *px++ = vecAddAcrossf64Mve(acc0);
        *px++ = vecAddAcrossf64Mve(acc1);
        *px++ = vecAddAcrossf64Mve(acc2);
        *px++ = vecAddAcrossf64Mve(acc3);
    ...
    if (row >= 2)
    {
        float64_t const    *pSrcA0Vec, *pSrcA1Vec, *pInVec;
        f64x4_t            vecIn, acc0, acc1;
        float64_t const     *pSrcVecPtr = pSrcVec;
    ...
    {
        f64x4_t             vecIn, acc0;
        float64_t const     *pSrcA0Vec, *pInVec;
        float64_t const      *pSrcVecPtr = pSrcVec;
        /*
...
ARM_DSP_ATTRIBUTE void arm_mat_vec_mult_f64(const arm_matrix_instance_f64 *pSrcMat, const float64_t *pVec, float64_t *pDst)
{
    uint32_t numRows = pSrcMat->numRows;
    uint32_t numCols = pSrcMat->numCols;
    const float64_t *pSrcA = pSrcMat->pData;
